...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.facePreviewView.contentMode = UIViewContentModeScaleAspectFit;
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    //NSLog(@"feature count = %d", (int)[features count]);
    if ([features count] > 0) {
        CIFaceFeature *ff = features[0];
        
        // -- adjust size of window we're going to capture, to handle 2 things:
        // 1 - Don't cut off chins. We'll widen the rectangle by 30%.
        // 2 - iOS feature detection is different than what OpenCV used. It tends to be smaller and a little lower down in
        // the face, so we're going to move the image up slightly.
        //NSLog(@"FaceCIImage size.width = %f", [FaceCIImage extent].size.width);
        //NSLog(@"FaceCIImage size.height = %f", [FaceCIImage extent].size.height);
        int widthAdj = ff.bounds.size.width * 0.3;
        int heightAdj = widthAdj;       // picture we upload must be a square. all the server side calcs depend on this.
        int newX = MAX(0, ff.bounds.origin.x - widthAdj/2);
        int newY = MAX(0, ff.bounds.origin.y - 0*heightAdj/2);
        int newW = MIN([FaceCIImage extent].size.width - newX, ff.bounds.size.width + widthAdj);
        int newH = MIN([FaceCIImage extent].size.height - newY, ff.bounds.size.height + heightAdj);
        if (newW != newH) {
            NSLog(@"%d not equal to %d, exiting", newW, newH);  // face is too close to camera, most likely.
            return;
        }
        CGRect aRect = CGRectMake(newX, newY, newW, newH);
        // ----
        CGImageRef cgImage = [_ciContext createCGImage:FaceCIImage fromRect:aRect];
        dispatch_async(dispatch_get_main_queue(), ^(void) {
//            double x = (_originalWidth - _originalHeight)/2;
            double x = 0;
            // this line makes the face frame a square box on the left hand side of the screen.
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    sv.FaceImage = _facePreviewView.image;
//    //[sv.FaceImage setTransform:CGAffineTransformMakeScale(1, -1)]
//    sv.FaceImage =  [[UIImage alloc] initWithCGImage: sv.FaceImage.CGImage
//                                               scale: 1.0
